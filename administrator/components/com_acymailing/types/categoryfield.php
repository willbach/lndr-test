...
class categoryfieldType{
	function display($table, $map, $previous){
		...
		$possibleCats[] = JHTML::_('select.option', '-1', acymailing_translation('ACY_NEW_CATEGORY'));
		if(!empty($allCats)){
	...
	function getFilter($table, $map, $previous, $js = ''){
		...
		$allCats = $db->loadObjectList();
		$possibleCats = array();
		$possibleCats[] = JHTML::_('select.option', 0, acymailing_translation('ACY_ALL_CATEGORIES'));
