...
class unsubType{
	function __construct(){
		...
		$this->values = array();
		$this->values[] = JHTML::_('select.option', '0', acymailing_translation('NO_UNSUB_MESSAGE') );
		foreach($messages as $oneMessage){
			$this->values[] = JHTML::_('select.option', $oneMessage->mailid, '['.acymailing_translation('ACY_ID').' '.$oneMessage->mailid.'] '.$oneMessage->subject);
		}
	...
	function display($value){
		...
		$text = ' <a '.$style.' class="modal" id="unsub_edit" title="'.acymailing_translation('EDIT_EMAIL',true).'"  href="'.$linkEdit.'" rel="{handler: \'iframe\', size:{x:800, y:500}}"><img class="icon16" src="'.ACYMAILING_IMAGES.'icons/icon-16-edit.png" alt="'.acymailing_translation('EDIT_EMAIL',true).'"/></a>';
		$text .= ' <a class="modal" id="unsub_add" title="'.acymailing_translation('CREATE_EMAIL',true).'"  href="'.$linkAdd.'" rel="{handler: \'iframe\', size:{x:800, y:500}}"><img class="icon16" src="'.ACYMAILING_IMAGES.'icons/icon-16-add.png" alt="'.acymailing_translation('CREATE_EMAIL',true).'"/></a>';
