...
class CpanelViewCpanel extends acymailingView{
	function display($tpl = null){
		...
		$loadLink = '<a onclick="window.document.getElementById(\'acymailing_messages_warning\').style.display = \'none\';return true;" class="modal" rel="{handler: \'iframe\', size:{x:800, y:500}}" href="index.php?option=com_acymailing&amp;tmpl=component&amp;ctrl=file&amp;task=latest&amp;code='.$language.'">'.acymailing_translation('LOAD_LATEST_LANGUAGE').'</a>';
		...
		if(acymailing_level(1)){
			...
			$forwardValues = array();
			$forwardValues[] = JHTML::_('select.option', 0, acymailing_translation('JOOMEXT_NO'));
			$forwardValues[] = JHTML::_('select.option', 1, acymailing_translation('JOOMEXT_YES'));
			$forwardValues[] = JHTML::_('select.option', 2, acymailing_translation('JOOMEXT_YES_FORWARD'));
		...
		if(acymailing_level(1)){
			...
			$queueType = array();
			$queueType[] = JHTML::_('select.option', 'onlyauto', acymailing_translation('AUTO_ONLY'));
			$queueType[] = JHTML::_('select.option', 'auto', acymailing_translation('AUTO_MAN'));
			$queueType[] = JHTML::_('select.option', 'manual', acymailing_translation('MANUAL_ONLY'));
		...
		$js = "function detectTimeout(id){
				try{
					...
					try{
						new Ajax('".rtrim(JURI::root(), '/')."/index.php?option=com_acymailing&tmpl=component&ctrl=statistics&task=detecttimeout&seckey=".$config->get('security_key')."',{ method: 'get', onComplete: function() { document.id(id).innerHTML = 'Done!'; window.document.getElementById(id).className = 'loading'; }}).request();
					}catch(err){
						new Request({url:'".rtrim(JURI::root(), '/')."/index.php?option=com_acymailing&tmpl=component&ctrl=statistics&task=detecttimeout&seckey=".$config->get('security_key')."',method: 'get', onComplete: function(response) { document.id(id).innerHTML = 'Done!'; window.document.getElementById(id).className = 'loading'; }}).send();
					}
		...
		foreach($cssval as $configval => $type){
			$myvals = array();
			$myvals[] = JHTML::_('select.option', '', acymailing_translation('ACY_NONE'));

			if($configval == 'css_backend'){
				$myvals[] = JHTML::_('select.option', 'backend_custom', acymailing_translation('ACY_CUSTOM'));
			...
			$elements->$configval .= ' <a id="'.$configval.'_link" '.$aStyle.' class="modal" title="'.acymailing_translation('ACY_EDIT', true).'"  href="'.$linkEdit.'" rel="{handler: \'iframe\', size:{x:800, y:500}}"><i class="acyicon-edit" style="margin: 5px 5px 0px 5px; display: inline-block;"></i></a>';
		}
		...
		$bootstrapFrontValues[] = JHTML::_('select.option', 0, acymailing_translation('JOOMEXT_NO'));
		...
		$menuvalues = array();
		$menuvalues[] = JHTML::_('select.option', '0', acymailing_translation('ACY_NONE'));
		$lastGroup = '';
		...
		$acyrss_format = array();
		$acyrss_format[] = JHTML::_('select.option', '', acymailing_translation('ACY_NONE'));
		$acyrss_format[] = JHTML::_('select.option', 'rss', 'RSS feed');
		$acyrss_format[] = JHTML::_('select.option', 'atom', 'Atom feed');
		$acyrss_format[] = JHTML::_('select.option', 'both', acymailing_translation('ACY_ALL'));
		...
		$acyrss_order = array();
		$acyrss_order[] = JHTML::_('select.option', 'senddate', acymailing_translation('SEND_DATE'));
		$acyrss_order[] = JHTML::_('select.option', 'mailid', acymailing_translation('ACY_ID'));
		$acyrss_order[] = JHTML::_('select.option', 'subject', acymailing_translation('ACY_TITLE'));
		...
		$elements->editConfEmail = '<a class="modal" id="confirmemail"  href="'.$link.'" rel="{handler: \'iframe\', size:{x:800, y:500}}"><button class="acymailing_button_grey" onclick="return false">'.acymailing_translation('EDIT_CONF_MAIL').'</button></a>';

		$link = 'index.php?option=com_acymailing&amp;tmpl=component&amp;ctrl=email&amp;task=edit&amp;mailid=notification_created';
		$elements->edit_notification_created = '<a class="modal" href="'.$link.'" rel="{handler: \'iframe\', size:{x:800, y:500}}"><button class="acymailing_button_grey" onclick="return false">'.acymailing_translation('EDIT_NOTIFICATION_MAIL').'</button></a>';
		$link = 'index.php?option=com_acymailing&amp;tmpl=component&amp;ctrl=email&amp;task=edit&amp;mailid=notification_refuse';
		$elements->edit_notification_refuse = '<a class="modal" href="'.$link.'" rel="{handler: \'iframe\', size:{x:800, y:500}}"><button class="acymailing_button_grey" onclick="return false">'.acymailing_translation('EDIT_NOTIFICATION_MAIL').'</button></a>';
		$link = 'index.php?option=com_acymailing&amp;tmpl=component&amp;ctrl=email&amp;task=edit&amp;mailid=notification_unsuball';
		$elements->edit_notification_unsuball = '<a class="modal" href="'.$link.'" rel="{handler: \'iframe\', size:{x:800, y:500}}"><button class="acymailing_button_grey" onclick="return false">'.acymailing_translation('EDIT_NOTIFICATION_MAIL').'</button></a>';
		$link = 'index.php?option=com_acymailing&amp;tmpl=component&amp;ctrl=email&amp;task=edit&amp;mailid=notification_unsub';
		$elements->edit_notification_unsub = '<a class="modal" href="'.$link.'" rel="{handler: \'iframe\', size:{x:800, y:500}}"><button class="acymailing_button_grey" onclick="return false">'.acymailing_translation('EDIT_NOTIFICATION_MAIL').'</button></a>';
		$link = 'index.php?option=com_acymailing&amp;tmpl=component&amp;ctrl=email&amp;task=edit&amp;mailid=notification_contact';
		$elements->edit_notification_contact = '<a class="modal" href="'.$link.'" rel="{handler: \'iframe\', size:{x:800, y:500}}"><button class="acymailing_button_grey" onclick="return false">'.acymailing_translation('EDIT_NOTIFICATION_MAIL').'</button></a>';
		$link = 'index.php?option=com_acymailing&amp;tmpl=component&amp;ctrl=email&amp;task=edit&amp;mailid=notification_contact_menu';
		$elements->edit_notification_contact_menu = '<a class="modal" href="'.$link.'" rel="{handler: \'iframe\', size:{x:800, y:500}}"><button class="acymailing_button_grey" onclick="return false">'.acymailing_translation('EDIT_NOTIFICATION_MAIL').'</button></a>';
		$link = 'index.php?option=com_acymailing&amp;tmpl=component&amp;ctrl=email&amp;task=edit&amp;mailid=notification_confirm';
		$elements->edit_notification_confirm = '<a class="modal" href="'.$link.'" rel="{handler: \'iframe\', size:{x:800, y:500}}"><button class="acymailing_button_grey" onclick="return false">'.acymailing_translation('EDIT_NOTIFICATION_MAIL').'</button></a>';

		$link = 'index.php?option=com_acymailing&amp;tmpl=component&amp;ctrl=email&amp;task=edit&amp;mailid=modif';
		$elements->editModifEmail = '<a class="modal" id="modifemail"  href="'.$link.'" rel="{handler: \'iframe\', size:{x:800, y:500}}"><button class="acymailing_button_grey" onclick="return false">'.acymailing_translation('EDIT_NOTIFICATION_MAIL').'</button></a>';

		$link = 'index.php?option=com_acymailing&amp;tmpl=component&amp;ctrl=cpanel&amp;task=checkDB';
		$elements->checkDB = '<a class="modal" href="'.$link.'" rel="{handler: \'iframe\', size:{x:800, y:500}}"><button class="acymailing_button_grey" onclick="return false">'.acymailing_translation('DATABASE_INTEGRITY').'</button></a>';
		...
		foreach($dirs as $dir){
			...
			if(!empty($languageFile)){
				...
				$oneLanguage->edit = ' <a class="modal" title="'.acymailing_translation('EDIT_LANGUAGE_FILE', true).'"  href="'.$linkEdit.'" rel="{handler: \'iframe\', size:{x:800, y:500}}"><i class="acyicon-edit" id="image'.$oneLanguage->language.'"></i></a>';
			}else{
				$linkEdit = 'index.php?option=com_acymailing&amp;tmpl=component&amp;ctrl=file&amp;task=language&amp;code='.$oneLanguage->language;
				$oneLanguage->edit = ' <a class="modal" title="'.acymailing_translation('ADD_LANGUAGE_FILE', true).'"  href="'.$linkEdit.'" rel="{handler: \'iframe\', size:{x:800, y:500}}"><i class="acyicon-new" id="image'.$oneLanguage->language.'"></i></a>';
			}
		...
		$allowmodif = array();
		$allowmodif[] = JHTML::_('select.option', "none", acymailing_translation('JOOMEXT_NO'));
		$allowmodif[] = JHTML::_('select.option', "data", acymailing_translation('ONLY_SUBSCRIPTION'));
		$allowmodif[] = JHTML::_('select.option', "all", acymailing_translation('JOOMEXT_YES'));
