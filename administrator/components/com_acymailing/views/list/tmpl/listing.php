	...
	<form action="index.php?option=<?php echo ACYMAILING_COMPONENT ?>&amp;ctrl=list" method="post" name="adminForm" id="adminForm">
		...
		<table class="acymailing_table" cellpadding="1" id="listListing">
			...
			<tr>
				...
				<th class="title">
					<?php echo JHTML::_('grid.sort', acymailing_translation('LIST_NAME'), 'a.name', $this->pageInfo->filter->order->dir, $this->pageInfo->filter->order->value); ?>
				</th>
				...
				<th class="title titlesender">
					<?php echo JHTML::_('grid.sort', acymailing_translation('CREATOR'), 'd.name', $this->pageInfo->filter->order->dir, $this->pageInfo->filter->order->value); ?>
				</th>
				<?php if(!ACYMAILING_J30){ ?>
					<th class="title titleorder">
						<?php echo JHTML::_('grid.sort', acymailing_translation('ACY_ORDERING'), 'a.ordering', $this->pageInfo->filter->order->dir, $this->pageInfo->filter->order->value);
				...
				<th class="title titletoggle">
					<?php echo JHTML::_('grid.sort', acymailing_translation('ENABLED'), 'a.published', $this->pageInfo->filter->order->dir, $this->pageInfo->filter->order->value); ?>
				</th>
				<th class="title titleid">
					<?php echo JHTML::_('grid.sort', acymailing_translation('ACY_ID'), 'a.listid', $this->pageInfo->filter->order->dir, $this->pageInfo->filter->order->value); ?>
				</th>
			</tr>
			</thead>
			<tfoot>
				...
				<tr class="<?php echo "row$k"; ?>">
					...
					<td align="center" style="text-align:center">
						...
						<?php if(!empty($row->nbwait)){
							...
							echo acymailing_tooltip(acymailing_translation('NB_PENDING'), ' ', 'tooltip.png', $title, acymailing_completeLink('subscriber&filter_status=0&filter_statuslist=2&filter_lists='.$row->listid)); ?>
						<?php } ?>
					</td>
					<td align="center" style="text-align:center">
						<a href="<?php echo acymailing_completeLink('subscriber&filter_status=0&filter_statuslist=-1&filter_lists='.$row->listid); ?>">
