...
function acymailing_setPageTitle($title){
	...
	if(empty($title)){
		...
		$title = acymailing_translation_sprintf('ACY_JPAGETITLE', $app->getCfg('sitename'), $title);
	}elseif($app->getCfg('sitename_pagetitles', 0) == 2){
		$title = acymailing_translation_sprintf('ACY_JPAGETITLE', $title, $app->getCfg('sitename'));
	}
...
function acymailing_listingsearch($search){
	...
	$searchBar .= '<button style="float:none;" onclick="document.adminForm.limitstart.value=0;this.form.submit();" class="btn tip hasTooltip" type="submit" title="'.acymailing_translation('ACY_SEARCH').'"><i class="acyicon-search"></i></button>';
	$searchBar .= '<button style="float:none;margin-left:0px;" onclick="document.adminForm.limitstart.value=0;document.getElementById(\'search\').value=\'\';this.form.submit();" class="btn tip hasTooltip" type="button" title="'.acymailing_translation('JOOMEXT_RESET').'"><i class="acyicon-cancel"></i></button>';
	$searchBar .= '</div>';
	echo $searchBar;
}
...
class Emoji
{
	...
	private static function convertEmoji($text,$op) {
		if(!file_exists(JPATH_SITE.DS.'plugins'.DS.'acymailing'.DS.'emojis')) return $text;
		if($op=="ENCODE"){
