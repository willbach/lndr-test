...
class acytabsHelper{
	...
	function panel($text, $id){
		...
		if($this->mode == 'pane'){
			...
			$ret .= JHtml::_('tabs.panel', acymailing_translation($text), $id);
		}else{
			if($this->openPanel){
				$this->_closePanel();
			}
	...
	function end(){
		...
		if($this->mode == 'pane'){
			...
			foreach($this->data as $k => $data){
				...
				$ret .= '	<li'.$active.'><a href="#'.$data->id.'" id="'.$data->id.'_tablink" data-toggle="tab">'.acymailing_translation($data->text).'</a></li>'."\r\n";
			}
