...
<div><?php if($this->config->get('frontend_subject',1)){ ?><h1 class="contentheading<?php echo $this->values->suffix; ?>"><?php echo $this->mail->subject; ?>
	<?php if($this->frontEndManagement AND ($this->config->get('frontend_modif',1) || ($this->mail->userid == $this->my->id)) AND ($this->config->get('frontend_modif_sent',1) || empty($this->mail->senddate))){ ?>
		<a <?php if(JRequest::getCmd('tmpl') == 'component') echo 'target="_blank" '; ?> href="<?php echo acymailing_completeLink('frontnewsletter&task=edit&mailid='.$this->mail->mailid.'&listid='.$this->list->listid); ?>"><img class="icon16" src="<?php echo ACYMAILING_IMAGES ?>icons/icon-16-edit.png" alt="<?php echo acymailing_translation('ACY_EDIT',true) ?>"/></a>
	<?php } ?>
</h1>
<?php } ?>
	...
	<?php $pdfimage = JHTML::_('image.site',  'pdf_button.png', !ACYMAILING_J16 ? '/images/M_images/' : '/media/system/images/', NULL, NULL, acymailing_translation( 'PDF' ) );
	...
	<?php }
		...
		<?php
		if(ACYMAILING_J30){
			$printimage = '<img src="'.ACYMAILING_IMAGES.'icons/icon-32-acyprint.png" alt="'.acymailing_translation( 'ACY_PRINT',true ).'" />';
		}else{
			$printimage = JHTML::_('image.site',  'printButton.png', !ACYMAILING_J16 ? '/images/M_images/' : '/media/system/images/', NULL, NULL, acymailing_translation( 'ACY_PRINT' ) );
		} ?>
