	...
	function listing(){
		...
		if(empty($allLists)){
			if(empty($my->id)){
				...
				$uri = JFactory::getURI();
				$app->redirect('index.php?option='.$usercomp.'&view=login&return='.base64_encode($uri->toString()), acymailing_translation('ACY_NOTALLOWED'));
			}else{
				$app->enqueueMessage(acymailing_translation('ACY_NOTALLOWED'), 'error');
		...
		$orderValues = array();
		$orderValues[] = JHTML::_('select.option', 'senddate', acymailing_translation('SEND_DATE'));
		$orderValues[] = JHTML::_('select.option', 'subject', acymailing_translation('JOOMEXT_SUBJECT'));
		$orderValues[] = JHTML::_('select.option', 'created', acymailing_translation('CREATED_DATE'));
		$orderValues[] = JHTML::_('select.option', 'mailid', acymailing_translation('ACY_ID'));
		...
		if($config->get('show_order', 1) == 1){
			...
			$orderDir = array();
			$orderDir[] = JHTML::_('select.option', 'ASC', acymailing_translation('ACY_ASC'));
			$orderDir[] = JHTML::_('select.option', 'DESC', acymailing_translation('ACY_DESC'));
