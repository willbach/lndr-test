...
if(!ACYMAILING_J16){
	class JElementCustomtemplate extends JElement
	{
		function fetchElement($name, $value, &$node, $control_name)
		{
			...
			$text = '<a class="modal" title="'.acymailing_translation('ACY_HELP',true).'"  href="'.$link.'" rel="{handler: \'iframe\', size: {x: 800, y: 500}}"><button class="btn" onclick="return false">'.acymailing_translation('ACY_CUSTOMTEMPLATE').'</button></a>';
			return $text;
		}
	}
}else{
	class JFormFieldCustomtemplate extends JFormField
	{
		...
		function getInput(){
			...
			$text = '<a class="modal" title="'.acymailing_translation('ACY_CUSTOMTEMPLATE').'"  href="'.$link.'" rel="{handler: \'iframe\', size: {x: 800, y: 500}}"><button class="btn" onclick="return false">'.acymailing_translation('ACY_CUSTOMTEMPLATE').'</button></a>';
			return $text;
		}
	}
}
