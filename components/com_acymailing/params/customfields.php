...
if(!ACYMAILING_J16){
	...
	class JElementCustomfields extends JElement
	{
		function fetchElement($name, $value, &$node, $control_name)
		{
			...
			$text .= '<a class="modal" id="link'.$control_name.'customfields" title="'.acymailing_translation('EXTRA_FIELDS').'"  href="'.$link.'" rel="{handler: \'iframe\', size: {x: 650, y: 375}}"><button class="btn" onclick="return false">'.acymailing_translation('Select').'</button></a>';
	...
	class JFormFieldCustomfields extends JFormField
	{
		...
		function getInput() {
			...
			$text .= '<a class="modal" id="linkcustomfields" title="'.acymailing_translation('EXTRA_FIELDS').'"  href="'.$link.'" rel="{handler: \'iframe\', size: {x: 650, y: 375}}"><button class="btn" onclick="return false">'.acymailing_translation('Select').'</button></a>';
