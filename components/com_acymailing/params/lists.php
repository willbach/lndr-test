...
if(!ACYMAILING_J16){
	...
	class JElementLists extends JElement
	{
		function fetchElement($name, $value, &$node, $control_name)
		{
			...
			$text .= '<a class="modal" id="link'.$control_name.$name.'" title="'.acymailing_translation('SELECT_LISTS').'"  href="'.$link.'" rel="{handler: \'iframe\', size: {x: 650, y: 375}}"><button class="btn" onclick="return false">'.acymailing_translation('Select').'</button></a>';
	...
	class JFormFieldLists extends JFormField
	{
		...
		function getInput() {
			...
			$text .= '<a class="modal" id="link'.$this->name.'" title="'.acymailing_translation('SELECT_LISTS').'"  href="'.$link.'" rel="{handler: \'iframe\', size: {x: 650, y: 375}}"><button class="btn" onclick="return false">'.acymailing_translation('Select').'</button></a>';
