...
if(!ACYMAILING_J16){
	class JElementPluginsfield extends JElement{
		function fetchElement($name, $value, &$node, $control_name){
			...
			$text = '<a class="modal" title="'.acymailing_translation('ACY_CONFIGURATION').'" href="'.$link.'" rel="{handler: \'iframe\', size: {x: 800, y: 500}}"><button class="btn" onclick="return false">'.acymailing_translation('ACY_CONFIGURATION').'</button></a>';
			return $text;
		}
	}
}else{
