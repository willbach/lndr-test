...
class KomentoProfile extends JUser
{
	...
	public function getAvatar( $email = '' )
	{
		...
		$vendorName	= $this->config->get('layout_avatar_integration');
	...
	{
		$useKomentoProfile = $this->config->get( 'use_komento_profile' );

		// If Komento uses Komento Profile, we skip getting the integration profile link.
		if ($useKomentoProfile) {

			if (!$this->id) {
				return 'javascript:void(0)';
			}
			return JRoute::_('index.php?option=com_komento&view=profile&id=' . $this->id);
		}
	...
	{
		if( !$this->config->get( 'enable_moderation' ) )
		{
			return false;
		}
