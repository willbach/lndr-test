		...
		<div class="kbody">
			<div class="search-user">
				<form action="<?php echo KunenaRoute::_('index.php?option=com_kunena&view=user&layout=list') ?>" name="usrlform" method="post">
					...
					<button class="btn btn-primary" type="submit" alt="<?php echo JText::_('COM_KUNENA_USRL_SEARCH'); ?>"><i class="fa fa-search"></i></button>
				</form>
			</div>
...
<div class="kblock">
	<div class="kheader">
		<span><?php echo JText::_('COM_KUNENA_USRL_USERLIST'); ?></span>
	</div>
			...
			<form action="<?php echo KunenaRoute::_('index.php?option=com_kunena&view=user&layout=list') ?>" method="post" id="kuserlist-form" name="kuserlist-form">
				...
				<table>
					<tr class="ksth userlist">
						...
						<th class="hidden-tablet hidden-phone"><?php echo JText::_('COM_KUNENA_USRL_ONLINE'); ?></th>
						<?php endif; ?>
						...
						<th class="usersortable hidden-tablet hidden-phone"><?php echo JHtml::_( 'kunenagrid.sort', 'COM_KUNENA_USRL_JOIN_DATE', 'registerDate', $this->state->get('list.direction'), $this->state->get('list.ordering'), '', '', 'kuserlist-form'); ?></th>
						<?php endif; ?>

						<?php if ($this->config->userlist_lastvisitdate) : ?>
						<th class="usersortable hidden-tablet hidden-phone"><?php echo JHtml::_( 'kunenagrid.sort', 'COM_KUNENA_USRL_LAST_LOGIN', 'lastvisitDate', $this->state->get('list.direction'), $this->state->get('list.ordering'), '', '', 'kuserlist-form'); ?></th>
						<?php endif; ?>
					...
					<tr class="k<?php echo $usrl_class ;?>">
						...
						<?php if ($this->config->userlist_online) : ?>
						<td class="kcol-mid hidden-tablet hidden-phone">
						...
						<?php if ($this->config->userlist_joindate) : ?>
						<td class="kcol-mid hidden-tablet hidden-phone" title="<?php echo KunenaDate::getInstance($user->registerDate)->toKunena('ago') ?>"><?php echo KunenaDate::getInstance($user->registerDate)->toKunena('datetime_today') ?></td>
						<?php endif; ?>

						<?php if ($this->config->userlist_lastvisitdate) : ?>
						<td class="kcol-mid hidden-tablet hidden-phone" title="<?php echo KunenaDate::getInstance($lastvisitDate)->toKunena('ago') ?>"><?php echo KunenaDate::getInstance($lastvisitDate)->toKunena('datetime_today') ?></td>
						<?php endif; ?>
