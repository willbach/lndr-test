...
<div class="kblock">
	<div class="kheader">
		<span><?php echo $this->escape($this->title)?></span>
	</div>
...
<table class="kblocktable<?php echo !empty ( $this->category->class_sfx ) ? ' kblocktable' . $this->escape($this->category->class_sfx) : ''?>" id="kpostmessage">
	<tbody id="kpost-message">
		...
		<tr id="kpost-category" class="krow<?php echo 1 + $this->k^=1 ?>">
			<td class="kcol-first hidden-tablet hidden-phone"><strong><?php echo JText::_('COM_KUNENA_CATEGORY')?></strong></td>
		...
		<tr class="krow<?php echo 1 + $this->k^=1 ?>" id="kanynomous-check" <?php if (!$this->category->allow_anonymous): ?>style="display:none;"<?php endif; ?>>
			<td class="kcol-first hidden-tablet hidden-phone">
		...
		<tr class = "krow<?php echo 1+ $this->k^=1 ?>">
			<td class = "kcol-first hidden-tablet hidden-phone"><strong><?php echo JText::_('COM_KUNENA_GEN_EMAIL');?></strong></td>
			<td class="kcol-mid">
		...
		<tr id="kpost-attachments" class="krow<?php echo 1 + $this->k^=1;?>">
			...
			<td class="kcol-mid">
				<div id="kattachment-id" class="kattachment">
					...
					<span class="kattachment-container">
					...
					<div class="kfile-hide hasTip" title="<?php echo JText::_('COM_KUNENA_FILE_EXTENSIONS_ALLOWED')?>::<?php echo $this->escape(implode(', ', $this->allowedExtensions)) ?>" >
						<button type="button" value="<?php echo  JText::_('COM_KUNENA_EDITOR_ADD_FILE'); ?>" class="kfile-input-button btn btn-primary"><?php echo  JText::_('COM_KUNENA_EDITOR_ADD_FILE'); ?></button>
						<input id="kupload" class="kfile-input" name="kattachment" type="file" />
					</div>
					<a href="#" class="kattachment-remove btn btn-primary  " style="display: none"><?php echo  JText::_('COM_KUNENA_GEN_REMOVE_FILE'); ?></a>
					<a href="#" class="kattachment-insert btn btn-primary  " style="display: none"><?php echo  JText::_('COM_KUNENA_EDITOR_INSERT'); ?></a>
					</span>
				</div>
		...
		<tr id="kpost-buttons" class="krow1">
			<td id="kpost-buttons" colspan="2">
				...
				title="<?php echo (JText::_('COM_KUNENA_EDITOR_HELPLINE_SUBMIT'));?>" tabindex="4" ><?php echo (' ' . JText::_('COM_KUNENA_SUBMIT') . ' ');?></button>
				...
				title="<?php echo (JText::_('COM_KUNENA_EDITOR_HELPLINE_PREVIEW'));?>:: "tabindex="3"><?php echo (' ' . JText::_('COM_KUNENA_PREVIEW') . ' ');?></button>
				<button type="button" name="cancel" class="btn btn-primary"
				value="<?php echo (' ' . JText::_('COM_KUNENA_CANCEL') . ' ');?>"
				onclick="window.history.back();"
				title="<?php echo (JText::_('COM_KUNENA_EDITOR_HELPLINE_CANCEL'));?>" tabindex="5"><?php echo (' ' . JText::_('COM_KUNENA_CANCEL') . ' ');?></button>
			</td>
		</tr>
	</tbody>
</table>
