...
if (!VmConfig::get('use_as_catalog', 0) ) { ?>
	...
	if (($stockhandle == 'disableit' or $stockhandle == 'disableadd') and ($product->product_in_stock - $product->product_ordered) < 1) { ?>
		<a title="<?php echo vmText::_ ('COM_VIRTUEMART_CART_NOTIFY') ?>" href="<?php echo JRoute::_ ('index.php?option=com_virtuemart&view=productdetails&layout=notify&virtuemart_product_id=' . $product->virtuemart_product_id); ?>" class="notify btn"><i class="material-icons-markunread "></i><span><?php echo vmText::_ ('COM_VIRTUEMART_CART_NOTIFY') ?>
</span></a>
		...
		if (!( VmConfig::get('askprice', true) and empty($tmpPrice) ) ) { ?>
			...
			if($customs !== 'select'){
				...
				echo JHtml::link($link.$ItemidStr,'<i class="material-icons-shopping_cart "></i><span>'.vmText::_ ( 'COM_SELECTOPTION' ).'</span>', array ('title' => vmText::_ ( 'COM_SELECTOPTION' ), 'class' => 'addtocart-button btn btn-primary' ) );
