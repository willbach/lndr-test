...
?>
<div class="product-price <?php echo $class; ?>" id="productPrice<?php echo $product->virtuemart_product_id ?>">
	<?php
	...
	if ($product->prices['salesPrice']<=0 and VmConfig::get ('askprice', 1) and isset($product->images[0]) and !$product->images[0]->file_is_downloadable) {
		...
		if ($product->prices['basePriceWithTax'] && round($product->prices['basePriceWithTax'],$currency->_priceConfig['salesPrice'][1]) != $product->prices['salesPrice']) {
