...
jQuery(function($) {
	...
	$(window).load(function(){
		if($('body').find('div').hasClass('mod-custom__page_loader')){
	...
	$('body').not('.com_config').find('input[type=radio]').not('.search_toggle').each(function(){if($(this).parent().not('span.radio')){if(!$(this).attr("id")){$(this).attr({id:'radio'+i}).wrap('<span class="radio"/>').after('<label class="radio_inner" for="radio'+i+'" />')}else{$(this).wrap('<span class="radio"/>').after('<label class="radio_inner" for="'+$(this).attr("id")+'" />')}}})
