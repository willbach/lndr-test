        ...
        if( !$app->isAdmin() )
        {
            ...
            if(!empty($matches[0])){
                foreach($matches[2] as $k => $match){
                    preg_match('|(<a[^>]*href[^>]*>)(.*)(</a>)|iUs', $match, $link);
                    if(!empty($link)){
                        $words = explode(' ', trim(strip_tags($link[2])));
                    }
                    else{
                    ...
                    $string = trim($string);
                    if(!empty($link)){
                        $string = $link[1] . $string . $link[3];
                    }
                    //$match = str_replace($matches[1][$k], $string, $matches[0][$k]);
                    $string = $matches[1][$k] . $string . $matches[3][$k];
                    $body = str_replace($matches[0][$k], $string, $body);
                }
            }
