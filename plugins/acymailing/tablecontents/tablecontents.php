...
class plgAcymailingTablecontents extends JPlugin{
	...
	function acymailingtablecontents_show(){
		...
		$contenttype = array();
		$contenttype[] = JHTML::_('select.option', '', acymailing_translation('ACY_EXISTINGANCHOR'));
		for($i = 1; $i < 6; $i++){
			$contenttype[] = JHTML::_('select.option', "|type:h".$i, 'H'.$i);
		}
		$contenttype[] = JHTML::_('select.option', 'class', acymailing_translation('CLASS_NAME'));

		$contentsubtype = array();
		$contentsubtype[] = JHTML::_('select.option', '', acymailing_translation('ACY_NONE'));
		for($i = 1; $i < 6; $i++){
			$contentsubtype[] = JHTML::_('select.option', "|subtype:h".$i, 'H'.$i);
		}
		$contentsubtype[] = JHTML::_('select.option', 'class', acymailing_translation('CLASS_NAME'));
